...
for ACCESSION in ${ACCESSIONS[@]}; do
  ...
  fastqc --threads 1 raw_reads/${ACCESSION}_R1.fastq.gz raw_reads/${ACCESSION}_R2.fastq.gz

  # Adapter/quality trim raw reads
  mkdir -p qc_reads
  trimmomatic PE \
      -threads 1 \
      raw_reads/${ACCESSION}_R1.fastq.gz raw_reads/${ACCESSION}_R2.fastq.gz \
      qc_reads/${ACCESSION}_R1.fastq.gz qc_reads/${ACCESSION}_R1.unpaired.fastq.gz \
      qc_reads/${ACCESSION}_R2.fastq.gz qc_reads/${ACCESSION}_R2.unpaired.fastq.gz \
  ...
  mkdir -p mapped
  bwa mem -t 1 \
    -R '@RG\tID:'${ACCESSION}'\tSM:'${ACCESSION} \
    references/reference.fasta.gz \
    qc_reads/${ACCESSION}_R1.fastq.gz qc_reads/${ACCESSION}_R2.fastq.gz \
  | samtools view -b \
